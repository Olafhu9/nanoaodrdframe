...
def function_calling_PostProcessor(outdir, rootfileshere, outtreename, intreeename, json, saveallbranches, globaltag):
    for afile in rootfileshere:
        ...
        subprocess.call(["./skimonefile.py", "--json=%s"%json, "--savealbranches=%s"%saveallbranches, "--globaltag=%s"%globaltag, afile, outfname, intreeename, outtreename])
    pass
        ...
        if len(rootfileshere)>0:
            ...
            if self.split>1: # use multiprocessing
                ...
                for afile in rootfileshere:
                    ...
                    subprocess.call(["./skimonefile.py", "--json=%s"%self.json, "--globaltag=%s"%self.globaltag, afile, outfname, self.intreeename, self.outtreename])
...
def Nanoaodprocessor_singledir(outputroot, indir, outtree, intree, json, split, recursive, saveallbranches, globaltag):
    ...
    for afile in rootfilestoprocess:
        t.Add(afile)
    aproc = ROOT.SkimEvents(t, outputroot, json, globaltag, split)
    aproc.setupAnalysis()
